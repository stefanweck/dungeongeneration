...
InteractionManager.prototype = {
	...
	onMouseMove: function(event) {
		...
		for(var i = 0; i < this.elements.length; i++){
			...
			if(this.mousePos.x >= this.elements[i].position.x && this.mousePos.x <= this.elements[i].position.x + this.elements[i].width && this.mousePos.y >= this.elements[i].position.y && this.mousePos.y <= this.elements[i].position.y + this.elements[i].height){
				...
				//We are not hovering the current element
