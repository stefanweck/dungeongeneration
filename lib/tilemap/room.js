		...
		for(var y = 0; y < this.h; y++) {
			...
			for(var x = 0; x < this.w; x++) {
				...
				if(y === 0 || y === this.h - 1 || x === 0 || x === this.w - 1) {
					...
					this.layout[y][x] = new Roguelike.Tile(1, true, this, 0, 1);

				}else{

					//Create a new floor tile
					this.layout[y][x] = new Roguelike.Tile(2, false, this, 0, Roguelike.Utils.randomNumber(2, 5));

					//TODO: Don't generate grass in a room, create a map decorator!
					//Have a random chance to spawn grass on this tile
					if(Roguelike.Utils.randomNumber(0, 100) >= 70){

						grassEntity = new Roguelike.DecorationFactory.newGrass({x: x, y: y});

						this.layout[y][x].entities.push(grassEntity);

						//Add the entity to the map
						//game.map.entities.addEntity(grassEntity);
