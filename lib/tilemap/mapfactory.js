...
Roguelike.MapFactory.prototype = {
	...
	generateVerticalCorridor: function(x, y) {
		...
		if(map.tiles[y + 1][x + 1].type === 0) {
			...
			//Set the current tile to wall
			this.changeTileToWall(map.tiles[y + 1][x + 1]);
	...
	placeDoor: function(position, orientation) {
		...
		if(orientation === true){
			console.log(doorEntity.components.sprite);
		...
		for(var y = 0; y < map.tilesY; y++) {
			...
			for(var x = 0; x < map.tilesX; x++) {
				...
				if(map.tiles[y][x].type === 1){
					...
					//Get the tile at the location of the possible door location
					var tileLeft = this.game.map.tiles[y][x - 1];
					var tileRight = this.game.map.tiles[y][x + 1];
					var tileDown = this.game.map.tiles[y + 1][x];
					var tileUp = this.game.map.tiles[y - 1][x];
