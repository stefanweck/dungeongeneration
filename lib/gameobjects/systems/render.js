	...
	drawMap: function() {
		...
		for(var y = 0; y < map.tilesY; y++) {
			...
			for(var x = 0; x < map.tilesX; x++) {
				...
				if(tileType == 2 && Roguelike.Utils.randomNumber(1, 100) >= 40){
					tileType = Roguelike.Utils.randomNumber(2, 5);
				}
				...
				if(tileType >= 2 && Roguelike.Utils.randomNumber(1, 100) >= 90){
					tile = Roguelike.Utils.randomNumber(3, 5);
					this.context.drawImage(
						tileSet,
						tile * 48,
						48,
				...
				if(tileType >= 2 && Roguelike.Utils.randomNumber(1, 100) >= 90){
					tileType = Roguelike.Utils.randomNumber(0, 5);
					this.context.drawImage(
						tileSet,
						tileType * 48,
						96,
						...
						48,
						(x * map.tileSize) - camera.position.x,
						(y * map.tileSize) - camera.position.y - 25,
						48,
						48
					);
				}
